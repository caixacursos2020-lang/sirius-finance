...
export default function AddExpensePage() {
  ...
  // Carrega métodos de pagamento para os botões
  ...
  // ESTADO DO PAGAMENTO SELECIONADO
  ...
  const handleCreateCategory = () => {
    ...
    if (existingCatIndex >= 0) {
      updatedCategories[existingCatIndex].subcategories.push({ id: newSubId, name: newSubName });
    } else {
    ...
    localStorage.setItem("sirius-price-research-categories", JSON.stringify(updatedCategories));
  ...
  function handleSubmit(e: FormEvent) {
    ...
    if (finalStore && !establishments.includes(finalStore)) {
      ...
      localStorage.setItem("sirius-price-research-establishments", JSON.stringify(newEst));
    }

    if (enablePriceTracking && priceCatId && priceSubId) {
      const existingEntries = JSON.parse(localStorage.getItem("sirius-price-research-entries") || "[]");
      ...
      localStorage.setItem("sirius-price-research-entries", JSON.stringify([newEntry, ...existingEntries]));
    }
  ...
  return (
    <div className="space-y-6 pb-20">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-semibold text-slate-100" style={TITLE_SHADOW}>Nova saída</h1>
          <p className="text-sm text-slate-400">Registre um novo gasto no seu controle financeiro.</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6 rounded-xl border border-slate-800 bg-slate-900 p-6">
        ...
        {/* --- SELETOR DE PAGAMENTO (MODERNO E ÚNICO) --- */}
        <div className="animate-in fade-in slide-in-from-left-2 duration-500">
          <label className="block text-xs font-medium text-slate-300 mb-2">Forma de Pagamento</label>
          ...
          <div className="rounded-lg border border-slate-800 bg-slate-950/70 p-4">
            ...
            <div className="grid gap-4 md:grid-cols-4">
              <div>
                ...
                <input type="text" value={fuelLiters} onChange={(e) => setFuelLiters(e.target.value)} className="mt-1 w-full rounded-md border border-slate-700 bg-slate-950 px-2 py-1.5 text-sm text-slate-100" />
              </div>
              <div>
                <label className="block text-xs font-medium text-slate-300">Preço/Litro</label>
                <input type="text" value={fuelPricePerLiter} onChange={(e) => setFuelPricePerLiter(e.target.value)} className="mt-1 w-full rounded-md border border-slate-700 bg-slate-950 px-2 py-1.5 text-sm text-slate-100" />
              </div>
              <div>
              ...
              <div>
                ...
                <input type="text" value={fuelStation} onChange={(e) => setFuelStation(e.target.value)} className="mt-1 w-full rounded-md border border-slate-700 bg-slate-950 px-2 py-1.5 text-sm text-slate-100" />
              </div>
            </div>
          </div>
        )}
        ...
        <div className="rounded-lg border border-slate-800 bg-slate-900/50 p-4 transition-all">
          <div className="flex items-center gap-3 mb-3">
            <button type="button" onClick={() => setEnablePriceTracking(!enablePriceTracking)} className={`flex items-center gap-2 text-sm font-medium transition-colors ${enablePriceTracking ? "text-emerald-400" : "text-slate-400 hover:text-slate-200"}`}>
              <div className={`flex h-4 w-4 items-center justify-center rounded border ${enablePriceTracking ? "bg-emerald-500 border-emerald-500" : "border-slate-600 bg-slate-900"}`}>
                {enablePriceTracking && <CheckCircle2 size={12} className="text-black" />}
              </div>
              ...
              {!isCreatingPriceCat ? (
                <div className="grid grid-cols-[1fr,1fr,auto] items-end gap-3">
                  <div>
                    ...
                    <select className="w-full rounded-md border border-slate-700 bg-slate-950 px-2 py-1.5 text-sm text-slate-100" value={priceCatId} onChange={(e) => { setPriceCatId(e.target.value); setPriceSubId(""); }}>
                      <option value="">Selecione...</option>
                      {priceCategories.map((cat) => <option key={cat.id} value={cat.id}>{cat.name}</option>)}
                    </select>
                  </div>
                  <div>
                    <label className="mb-1 block text-xs font-medium text-slate-300">Item</label>
                    <select className="w-full rounded-md border border-slate-700 bg-slate-950 px-2 py-1.5 text-sm text-slate-100" value={priceSubId} onChange={(e) => setPriceSubId(e.target.value)} disabled={!selectedPriceCategory}>
                      <option value="">Selecione...</option>
                      {selectedPriceCategory?.subcategories.map((sub) => <option key={sub.id} value={sub.id}>{sub.name}</option>)}
                    </select>
                  </div>
                  <button type="button" onClick={() => setIsCreatingPriceCat(true)} className="flex items-center gap-1.5 rounded-md border border-slate-700 bg-slate-800 px-3 py-2 text-xs text-emerald-400 hover:border-emerald-500 sm:text-sm"><Plus size={16} /> <span>Criar</span></button>
                </div>
              ) : (
                <div className="flex flex-col gap-3 rounded-lg border border-slate-700 bg-slate-800/50 p-3">
                  <div className="flex items-center justify-between"><span className="text-xs font-bold text-emerald-400">Criar Nova Categoria de Preço</span><button onClick={() => setIsCreatingPriceCat(false)}><X size={14} className="text-slate-400" /></button></div>
                  <div className="grid grid-cols-2 gap-3">
                    <input type="text" placeholder="Ex: Limpeza" value={newCatName} onChange={(e) => setNewCatName(e.target.value)} className="rounded border border-slate-600 bg-slate-950 px-2 py-1 text-sm text-white" />
                    <input type="text" placeholder="Ex: Sabão Líquido" value={newSubName} onChange={(e) => setNewSubName(e.target.value)} className="rounded border border-slate-600 bg-slate-950 px-2 py-1 text-sm text-white" />
                  </div>
                  <button type="button" onClick={handleCreateCategory} disabled={!newCatName || !newSubName} className="flex w-full items-center justify-center gap-2 rounded bg-emerald-600 py-1.5 text-xs font-bold text-white hover:bg-emerald-500 disabled:opacity-50"><Save size={14} /> <span>Salvar e Selecionar</span></button>
                </div>
              )}
            </div>
          )}
        ...
        <div className="flex justify-end gap-3 pt-2">
          ...
          <button type="submit" className="rounded-md bg-emerald-600 px-4 py-2 text-sm font-semibold text-slate-900 hover:bg-emerald-500">Salvar saída</button>
        </div>
      </form>
    </div>
  );
